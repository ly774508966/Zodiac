...
{
	sealed class ZodiacVectorItem : ZodiacPropertyItem
	{
		[SerializeField]
		GameObject ZGroup;
		...
		void UpdateInputFields ()
		{
			...
			case SerializedPropertyType.Vector3:
				...
				vector = (Vector3)memberMono.GetBoxedData ();
				count = 3;
				break;
			case SerializedPropertyType.Vector4:
		...
		public bool SetValue (string input, int index)
		{
			...
			case SerializedPropertyType.Vector2:
				...
				if (!Mathf.Approximately (vector2 [index], value)) {
					...
					memberMono.SetBoxedData (vector2);
					UpdateInputFields ();
					return true;
				}
				break;
			case SerializedPropertyType.Vector3:
				...
				if (!Mathf.Approximately (vector3 [index], value)) {
					...
					memberMono.SetBoxedData (vector3);
					UpdateInputFields ();
					return true;
				}
				break;
			case SerializedPropertyType.Vector4:
				...
				if (!Mathf.Approximately (vector4 [index], value)) {
					...
					memberMono.SetBoxedData (vector4);
					UpdateInputFields ();
					return true;
				}
				break;
